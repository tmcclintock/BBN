...
void initializeAll(double *T9,double *h,double *phie,double *t,double *Y){
  /*Initialize T9, h, and time and set rhob0*/
  *T9 = getTInit();
  *h=gethInit();
  *t=getTimeInit(*T9);
  setrhob0(*T9,*h);

  /*Initialize all abundances*/
...
void initializeConstants(double *deltaM,double *Z,double *Qvals,double *A,double *nni,double *nnj,double *nnk,double *nnl, double reactionDetails[totalnreac][8]){
  ...
  for(i=0;i<totalnreac;i++){
    for(j=0;j<8;j++){
      reactionDetails[i][j] = reacparam[i][j];
    }
  }//reactionDetails now contains all of the information for the reactions
