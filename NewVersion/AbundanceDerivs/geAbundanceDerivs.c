#include "geAbundanceDerivs.h"

/* This finds the abundance changes using gaussian elimination (ge) to find the next set of 
   abundances. This is in no way affected by the fact that we are using RK4 or RK2,
   aside from the fact that error correcting isn't as necessary in doing RK4.
*/
...
int getdYdt(double h,double T9,double H,int ki,int inc,int ip,double *Y0,double *Y,double *dYdt,double dt){
  /*Y0 is the abundances at the start of the current step, while Y is the abundances
    at the current RK4 part. I.E. Y = Y(t), Y(t+dt/2), Y(t+dt/2) or Y(t+dt), where the
    middle two represent Y at k2 and k3, respectively.
    ki is analogous to the loop variable in Kawano's sol routine.
  */

  /* Recieve forward reaction rates */
  double f[totalnreac],r[totalnreac];
  reactionRates(T9,f,r);
...
/*   } */
  ...
  for(i=0;i<nreac;i++){
    type[i] =(int)reactionDetails[i][1];
    ni[i]   =(int)reactionDetails[i][2];
    nj[i]   =(int)reactionDetails[i][3];
    nk[i]   =(int)reactionDetails[i][4];
    nl[i]   =(int)reactionDetails[i][5];
    rev[i]  =reactionDetails[i][6];
    q9[i]   =reactionDetails[i][7];
  }
  ...
  for(n=0;n<nreac;n++){
	...
	case 10:{ /* (2,0,2,1) type */
      ...
      /* Rever indices when adding them to the a[][] array
  ...
  for(i=0;i<nnuc;i++){
    yY[i]=yx[nnuc-1-i];
    dYdt[i]=(yY[i]-Y0[i])/dt;
/*     if(T9>99.9)printf("Y[%i]=%e\tdYdt[%i]=%e\tdt=%e\n",i,Y0[i],i,dYdt[i],dt); */
  }
